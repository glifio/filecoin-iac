...
data "aws_secretsmanager_secret" "slack_monitoring_dev_channel" {
  count = local.enable_notifications

  name = "${module.generator.prefix}/slack-monitoring-dev-channel"
}

data "aws_secretsmanager_secret_version" "slack_monitoring_dev_channel" {
  count = local.enable_notifications

  secret_id = data.aws_secretsmanager_secret.slack_monitoring_dev_channel[0].id
}