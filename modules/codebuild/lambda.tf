...
resource "aws_lambda_function" "slack_notif" {
  ...
  environment {
    variables = {
      "WEBHOOK_URL"  = jsondecode(data.aws_secretsmanager_secret_version.slack_monitoring_channel[0].secret_string)["webhook_url"]
      "CHANNEL_NAME" = jsondecode(data.aws_secretsmanager_secret_version.slack_monitoring_channel[0].secret_string)["channel_name"]
    }
  }
}
