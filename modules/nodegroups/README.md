...
- Creating a secret in AWS  and importing it to Kubernetes
- Creating an ingress
...
- Set of variables to create and using a new secret
```
create_aws_secret = true (by default)
create_k8s_secret = true (by default)
```

- Set of variables to create secret and to import existing AWS secret data
```
create_aws_secret = true (by default)
create_k8s_secret = true (by default)
...
````
enable_public_access = true
use_auth_token = false (by default)
enable_path_transformer = true (by default)
enable_return_json = true (by default)
enable_cors = true (by default)
````
...
````
enable_public_access = false (by default)
...
- If enable_public_access = false (the default value)  add Authorization header with value false to the request.
- If enable_public_access = true (the default value) and `var.use_auth_token = true` add Authorization header with the valid JWT token from AWS Secrets Manager to the request.
...
module "eks_nodegroup_ondemand_${name}" {
  ...
  get_namespace                           = kubernetes_namespace_v1.network.metadata[0].name
    ...
    # create secret #
   from_secret = "filecoin-mainnet-apn1-glif-calibration" 
...
| <a name="input_use_auth_token"></a> [use\_auth\_token](#input\_use\_auth\_token)                                                                                | If true, kong plugin using to section with token the `Authorization: Bearer auth_token`                                                 | `bool` | `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | no |
